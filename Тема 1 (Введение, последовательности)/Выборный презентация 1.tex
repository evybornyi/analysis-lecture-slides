...
\text{$n$ делится на 2}.
$$
Получили, что $n$ и $m$ оба делятся на 2. Это противоречит тому, что $n/m$ --- несократимая дробь.
\end{block}
\end{frame}
...
\item Окрестность $O(a)$ точки $a$ --- это произвольный интервал, содержащий точку $a$.
...
Число $a$ называют {\bf пределом последовательности} $a_n$, если все члены последовательности $a_n$, начиная с некоторого номера $N$, лежат в $\varepsilon$-окрестности точки $a$, для любого наперед заданного числа $\varepsilon>0$:
